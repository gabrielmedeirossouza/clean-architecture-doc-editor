...
export const Observable = class <T> implements Observable<T> {
    private _observers: Array<IFn<T>> = [];

    public Subscribe<J extends IFn<T>>(observer: J): Result<J, string> {
    ...
    public Unsubscribe(observer: IFn<T>): Result<void, string> {
